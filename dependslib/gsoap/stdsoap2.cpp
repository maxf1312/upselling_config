...
ssl_auth_init(struct soap *soap)
{
  ...
  if (!soap->ctx)
  {
    ...
    if (!soap->ctx)
	{
		...
		//	ERR_print_errors_fp( soap->fdebug[SOAP_INDEX_TEST] ), 
		//	fflush(soap->fdebug[SOAP_INDEX_TEST]);
		
		// TODO Move this workaround from library code to custom ssl_auth callback
		SSL_library_init();
		OpenSSL_add_all_algorithms(); /* we keep ciphers and digests for the program's lifetime */
#ifndef WITH_LEAN
		SSL_load_error_strings();
