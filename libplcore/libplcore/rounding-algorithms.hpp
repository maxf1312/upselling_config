...
namespace rounding
  ...
  FloatType roundhalfeven(
	...
	if (std::fabs(value -(ipart +0.5)) < epsilon)
      {
  ...
  bool _is_up = false;
  template <typename FloatType>
  FloatType roundalternate( const FloatType& value, int const& is_up = _is_up )
    {
    if ((is_up != _is_up))
  ...
  // Bias: none for sequential calls
  template <typename FloatType>
  FloatType roundalternate0( const FloatType& value, int const& is_up = _is_up )
    {
    if ((is_up != _is_up))
      return roundhalfup0( value );
